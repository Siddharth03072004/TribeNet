  ...
  useEffect(() => {
    ...
    if (
      authState.connections.some(
        (user) => user.connection.id === userProfile.user.id
      )
    ) {
    ...
    if (
      authState.connectionRequests.some(
        (user) => user.user.id === userProfile.user.id
      )
    ) {
      setIsCurrentUserInConnection(true);
      if (
            ...
            <div style={{ display: "flex", gap: "0.7rem" }}>
              <div style={{ flex: "0.8" }}>
                ...
                >
                  ...
                  <p style={{ color: "grey" }}>@{userProfile.user.username}</p>
                </div>
                  ...
                  <div
                    onClick={async () => {
                      const response = await clientServer.get(
                        `/user/download_resume?id=${userProfile.user.id}`
                      );
                      window.open(
